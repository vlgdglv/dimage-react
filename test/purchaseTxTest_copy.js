...
contract('Purchase transaction: make first deal test ', ([deployer, purchaser, owner, author, nobody])=>{
  ...
  beforeEach(async() => {
    ...
    const result = await instance.deploy({
      ...
      arguments:[releaseAddress, 1, purchaser, 360, "0x1234"],
  ...
  describe('transactions check:', async() => {
    ...
    it('decline purchase', async() => {
      ...
      const result = await contract.methods.declinePurchase().send({ from: owner, gasLimit:250000 })
    ...
    it('cancel purchase', async() => {
      ...
      const result = await contract.methods.cancelPurchase().send({ from: purchaser, gasLimit:250000 })
...
contract('Purchase transaction: continuous purchase test', ([purchaser, owner, author, nobody])=>{
  const gasLimit = 30000000
  const offer = 2000000000000000;
  let instance, address;
  let originPurchaseBalance;
  ...
  describe('transactions check:', async() => {
    ...
    it('second deal', async() => {
      ...
      newResult = await instance.deploy({
        data: Purchase.bytecode,
        arguments:[releaseAddress, 1, nobody,36000, "0x1234"],
